...
import { CalendarDays, LayoutList, Calendar as CalendarIcon, CheckCircle2, AlertCircle, ChevronLeft, ChevronRight, View, Clock, FileText, CalendarRange, Grid } from 'lucide-react';
...
export default function CalendarPage() {
    ...
    const calendarDays = eachDayOfInterval({ start: startDate, end: addDays(startDate, 41) }).slice(0, 42); // 6 weeks fixed grid
        ...
        invoices?.filter(i => ['en_attente', 'en_retard'].includes(i.status)).forEach(i => {
            if (i.due_date) {
                items.push({
                    ...
                    title: `FA-${i.id.split('-')[0].toUpperCase()}`, // Shorter title for month view
    ...
    const getItemsForDate = (date: Date) => {
        return allItems.filter(item => isSameDay(item.date, date));
    };
    ...
    return (
        <div className="space-y-4 h-[calc(100vh-120px)] flex flex-col">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
                <TabsList className="w-full sm:w-auto self-start bg-surface border border-border">
                    <TabsTrigger value="calendar" className="data-[state=active]:bg-brand-light data-[state=active]:text-brand">
                        <CalendarDays className="h-4 w-4 mr-2" />
                        Agenda
                    </TabsTrigger>
                    <TabsTrigger value="kanban" className="data-[state=active]:bg-brand data-[state=active]:text-white">
                        <LayoutList className="h-4 w-4 mr-2" />
                        Kanban des tâches
                    </TabsTrigger>
                </TabsList>

                <TabsContent value="calendar" className="flex-1 mt-4 focus-visible:outline-none focus-visible:ring-0 flex flex-col overflow-hidden bg-white border border-border rounded-xl shadow-sm">
                    {/* Calendar Toolbar */}
                                        ...
                                        {getItemsForDate(baseDate).length === 0 ? (
                                            <div className="flex flex-col items-center justify-center py-20 text-center text-text-muted">
                                                <div className="bg-surface2 p-4 rounded-full mb-4">
                                                    <CheckCircle2 className="h-10 w-10 text-p1/50" />
                                                </div>
                                                <p className="font-serif text-xl text-text-primary mb-1">Journée libre !</p>
                                                <p className="text-sm">Aucune échéance ou tâche de planifiée pour aujourd'hui.</p>
                                            </div>
                                        ) : (
                ...
                <TabsContent value="kanban" className="flex-1 mt-4 focus-visible:outline-none focus-visible:ring-0 overflow-hidden h-full">
                    <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm h-full overflow-hidden">
                        <ProjectKanban />
                    </div>
                </TabsContent>
            </Tabs >
